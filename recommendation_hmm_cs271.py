...
Outputs (out_hmm/):
  - observations.npz
  - hmm_model.npz
  - accuracy_by_movies.csv
  - accuracy_by_rows.csv
  - eval_top1_last_movie.csv
...
class HMMConfig:
    ...
    ratings_path: Path = Path("hmm_training_ratings.csv")
...
class HMMBackend:
    ...
    def __init__(self, cfg: HMMConfig, vocab: Vocabulary):
        ...
        self.vocab = vocab
        self.model = None
        self.M: Optional[int] = None  # number of tokens
        self.training_history = None  # EM log-likelihood history
    ...
    def fit(self, X_tokens: np.ndarray, lengths: List[int]) -> None:
        ...
        if hasattr(self.model, "monitor_"):
            self.training_history = getattr(self.model.monitor_, "history", None)
    ...
    def save_npz(self, path: Path) -> None:
        if self.model is None:
            raise RuntimeError("Model not fitted.")
        np.savez_compressed(path, transmat=self.model.transmat_,
                            ...
                            training_history=(np.array(self.training_history, dtype=float)
...
class Recommender:
    ...
    def __init__(self, backend: HMMBackend, vocab: Vocabulary,
        ...
        # cache for "seen" movies per user (optional)
        self._seen_cache: Dict[int, set[int]] = {}
    ...
    def predict_genre_mass(self, user_id: int, use_train_only: bool = True,
        ...
        if gamma_user > 0.0:
            ...
            # scale gamma_user by history length (saturate at 500 rows)
            max_hist = 500.0
    ...
    def topN(self, user_id: int, N: int, use_train_only: bool = True,
        ...
        # use cache, but also populate it
        if user_id not in self._seen_cache:
            self._seen_cache[user_id] = set(u["movieId"].unique())
        seen = self._seen_cache[user_id]
...
class Pipeline:
    ...
    def _tune_n_states(self, candidates=(4, 8, 12, 16, 20)) -> int:
    ...
    def save_metric_tables(self, evaluator: "Evaluator") -> None:
        out = self.cfg.outdir
    ...
    def run_all(self, target_user_id: Optional[int] = None) -> None:
        ...
        # quick tune
        self.cfg.n_states = self._tune_n_states(candidates=(8, 12, 16))
        ...
        # Save metric tables for later plotting (no matplotlib here)
        self.save_metric_tables(evaluator)
        ...
        np.savez_compressed(
            self.cfg.outdir / "observations.npz",
            X=X_train,
